...
class CnnMnist(nn.Module, BaseMnist):
    def __init__(
        self,
        n_epochs: int = 10,
        ...
        self.conv0 = nn.Conv2d(in_channels=1, out_channels=6, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2))
        self.activation0 = nn.ReLU()
        self.maxpool0 = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        # second convolutional layers
        self.conv1 = nn.Conv2d(in_channels=6, out_channels=16, kernel_size=(5, 5), stride=(1, 1))
        self.activation1 = nn.ReLU()
        self.maxpool1 = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))

        # first fully-connected network
        self.flatten = nn.Flatten()
        self.linear2 = nn.Linear(in_features=400, out_features=120)
    ...
    def forward(self, x):
        conv0 = self.conv0(x)
        activation0 = self.activation0(conv0)
        maxpool0 = self.maxpool0(activation0)

        conv1 = self.conv1(maxpool0)
        activation1 = self.activation1(conv1)
        maxpool1 = self.maxpool1(activation1)

        flatten = self.flatten(maxpool1)
        linear2 = self.linear2(flatten)
        activation2 = self.activation2(linear2)

        linear3 = self.linear3(activation2)
        ...
        for epoch in range(self.n_epochs):
            if test_loader is not None:
                ...
                for test_batch_idx, test_data in enumerate(test_loader):
                    ...
                    test_outputs = self.forward(torch.unsqueeze(test_inputs, dim=1))
                    loss_val = self.loss_function(test_outputs, test_targets)
                    test_loss += loss_val.item()
                    output_class_probabilities = self.class_probabilities(test_outputs)
            ...
            for batch_idx, train_data in enumerate(train_loader):
                ...
                train_outputs = self.forward(torch.unsqueeze(train_inputs, dim=1))
                loss_val = self.loss_function(train_outputs, train_targets)
                self.optimizer.step()
                train_loss += loss_val.item()
                output_class_probabilities = self.class_probabilities(train_outputs)
            ...
            if self.epoch_to_save_model is not None and (epoch % self.epoch_to_save_model == 0 and epoch > 0):
                ...
                torch.save(self.state_dict(), epoch_output_path)
...
def cnn_mnist_run(
    ...
    if train:
        ...
        cnn_mnist.train_model(train_loader, get_real_path(model_output_base_path), test_loader)
